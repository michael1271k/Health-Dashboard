...
if [ "$SCREEN" = "all" ]; then
  ...
  SCREENS=(signin backfill today today-edit today-sheet today-sheet-vitals today-weighin today-board train logger logger-finish day day-rows day-empty day-inbody day-swap doms fuel fuel-over fuel-empty nutrients macro-edit you levers sync-status sync-doctor plan body volume library exercise reports report history history-week session session-ledger exercise-history trends trends-empty body-trends body-trends-empty widgets)
fi
