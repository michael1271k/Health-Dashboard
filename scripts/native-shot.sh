...
if [ "$SCREEN" = "all" ]; then
  ...
  SCREENS=(signin backfill today today-edit today-sheet today-sheet-vitals train logger logger-finish day day-empty day-inbody day-swap fuel fuel-over fuel-empty nutrients macro-edit you levers plan body volume library exercise reports report history session session-ledger exercise-history trends trends-empty body-trends body-trends-empty pathfinder pathfinder-empty widgets)
fi
