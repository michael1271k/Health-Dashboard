...
**As shipped (2026-09-04).** `native/Packages/HelixUI` is the new package (design system moved in, generated atlas moved in, ten tile files lifted with drawing code unchanged apart from tokens; `HelixSnapshot.sample` + the `widgets` harness screen render all 67 cells with no network). `HelixSnapshot` lives in HelixCore; `WidgetSnapshotBuilder` (HelixData) ports the whole snapshot route onto the mirror; `AppDatabase.sharedFolder()/readOnly()/onCommit()/knownUserId()` are the App Group read path and the app reloads timelines 2 s after any commit. `HelixIntents` kept its five configurations (the `AppEnum` pickers are extension-local `*Option` enums bridging to the HelixUI focus enums — the AppIntents extractor refuses conformances on imported types). Deep links: `DeepLink.safePath`/`destination(forPath:)` in HelixCore, 30 golden vectors, `.onOpenURL` on the tab view; `/day/<iso>` switches to Body but does not yet open the date. The Capacitor extension, its three App-target bridges, `/api/widget/snapshot`, `widget_tokens` plumbing and six parity tests are deleted; the watch keeps `Legacy/` copies of the old snapshot + palette so the `App` scheme still builds, with no data source until Wave 10. **The App Group entitlement is in `project.yml` for both targets and needs the paid programme to sign; on a free team the app falls back to Application Support and the widgets stay empty.**
...
**As shipped (2026-09-04).** `native/HelixNative/Features/Today/` is the tab. The tiles ARE the widget faces — `HelixTile.face(id, entry:)` in HelixUI maps a `WidgetId` to a family + focus, so the grid and the Home Screen share one drawing; `StepsView` and `MuscleView` (atlas tinted by `volumeByFamily`) are the only new faces. **Five catalogue widgets have no face yet** (`deficit`, `bar`, `micros`, `stack`, `fatigue`): they are projected out of a slot at draw time (`WidgetId.isNative`), never offered in the gallery, and left in the stored layout for the web and for Wave 7. Layout: `AppDatabase.dashboardLayoutStream` / `saveDashboardLayout` (v4 payload, desktop side carried through unparsed, outbox upsert keyed `user_id`). Feed: `TodayFeedBuilder` = the widget snapshot + `Insights.compute` + week totals + the weekly-summary flag, rebuilt 300 ms after any commit. Grid: explicit row packing (`DashboardGrid.rows`; `gridCellColumns` does not work in `LazyVGrid`), `.draggable`/`.dropDestination`, 600 ms hover over a same-size tile to stack; jiggle ±0.8° with the −/size badges, none under Reduce Motion. Stacks: `TabView(.page)` rotated a quarter turn so swipe-up is forward, 9 s clock with the web's Java-hash stagger, paused in edit mode and off-foreground, a manual swipe holds the clock one period; Edit Stack is a `List` with `onMove`. Sheets: six domain extras on top of the tile's Large face, `[.medium, .large]`, `presentationBackground(GlassLevel.sheet.material)`. Two new vectors: `week-so-far.json` (`biggestChange`, lifted out of `WeekSoFarCard.tsx`) and `readiness-schedule.json` (`scheduleAwareReadiness`, lifted out of `useInsights.ts`). Shots: `today`, `today-edit`, `today-sheet` (+ AX5). Known: the Well-being Medium face truncates its captions at the app's tile width — Wave 5 drawing, untouched.
...
**As shipped (2026-09-04).** The kit is `HelixUI/Charts/HelixChart.swift`: one `helixChart(_ domain)` modifier (hairline grid, no axis line, tertiary rounded-monospaced labels, legend on top from `chartForegroundStyleScale`, clear plot, animation cut under Reduce Motion), `helixScrollable(days:)` as the range rail (`chartScrollableAxes` + `chartXVisibleDomain`, parked on the newest data), `HelixCallout` as the `chartXSelection` annotation, `HelixChartCard` (plot height is a `@ScaledMetric`; the header stacks at accessibility sizes) and `HelixChartEmpty`. `Color.helix.series` is a FIXED six-colour categorical order validated on black with the dataviz palette script (OKLCH L 0.48–0.67, worst adjacent CVD ΔE 9.1, ≥ 3:1); a seventh series is the neutral. Nothing from `ChartTooltip`/`SmartLegend`/`ChartRange`/`parts.tsx` was ported. Four doors: **History** and **Trends** from the Train toolbar, **Trends** from the Body toolbar, **Pathfinder** under You → Plan. `HelixData/History/` holds `read(_:)` + `seedRows(_:)` (range reads shaped at the call site — a named stream per chart would be a query-key layer by another name) and four query-only stores. *Workout Analysis:* `SessionHistoryView` (months, day dot, tonnage/sets/min/PR) → `SessionDetailView` (hero, a `Section` per exercise with the previous session's set beside each row via `SessionDetail.rowsWithPrev`, PR axis chips, the `progressionCue` in the header, `exerciseStats` in the footer, highlights, muscle focus as ranked bars, cardio, a Progression Trail chart); PRs are replayed chronologically through `PrEngine.buildBaselines`/`detectSessionPrs` with `set_type`/`side`/`pair_id` carried, keyed by `exercise_id` as `save.ts` does. `ExerciseDetailView` gained Records (`personal_records`, `PrEngine.axisLabel`), the est-1RM trend (`WidgetDerive.e1rmTrends` = `collapseToSessionBest`, stored `est_1rm_kg > 0` else Epley) and a 40-set ledger. *Trends:* stacked weekly tonnage by `VolumeSplit` (bars tinted `Color.helix.day`, native legend, y from zero), the 12-week `IntensityCalendar` as a `RectangleMark` heatmap on one accent at rising opacity, session-best est-1RM for the top four lifts in `Color.helix.series` order with a multi-line callout, 28-day sets by family from `MuscleAggregator`. *Body & Vitals:* one composition chart with a seven-metric `Picker` (never a second axis), dashed target `RuleMark`, ledger with `DeltaVerdict` colouring, steps bars against the goal, and `VitalsGroups.tsx`'s `MetricDef` table ported as data (`VitalMetrics.swift`: Recovery on Lunar, Respiratory/Engine/Rhythm on Tide; zero is "no reading" for the autonomic four; missing days are gaps). *Pathfinder:* week nodes newest-first from the first phase start (`Week.label`, phase badge, logged/planned, tonnage, a hidden-axis sparkline, chevron to `ReportReaderView` when a report covers the week), era `Picker`, swipe → "Copy export" (`WeeklyExportBuilder.input(weekStart:)` → `WeeklyExport.build` → `UIPasteboard`), context menu "Copy JSON" (`WeekJson`). The builder reads the 18 tables `useWeeklyExport.ts` reads with the same filters; where the mirror lacks a column the web had (`workout_sets.is_pr`, `quality`, `workout_sessions.split_day`, `avg_bpm`, `calories_burned`) it reconstructs from `personal_records` / `sessionVolumeKg` / the day key and never invents. Gate: `WeeklyExportBuilderTests` seeds one real week and asserts the assembled `WeeklyExportInput` section-by-section (the fixture's rich case is a hand-made renderer input and cannot be re-seeded); the string vectors stay HelixCore's. Shots: `history`, `session`, `exercise-history`, `trends(-empty)`, `body-trends(-empty)`, `pathfinder(-empty)` + AX5. **Known:** HelixCore's `Detail*`/`Export*` types have no public inits, so `History/CoreBridge.swift` and the builder's `make` construct them through `Codable` — Track D adds the inits and both helpers die. The web's ⬆️/⬇️ `compareProgress` glyph is unported (Track D). The five face-less widgets from Wave 6 are still projected out. `plan_phase_volume` targets are not drawn on the muscle chart (its key is not the six-family name).
