...
**W4 gave it a writer, and fixed the slot vocabulary.** The Head row on Pulse (`PulseHead.swift` → `AppDatabase.setStress`) files a reading under one of three time-of-day buckets — `morning` (before 12:00), `midday` (before 18:00), `evening` — chosen from the clock, never by the user; a day that has already passed files under `evening`. So a day carries at most three rows, one per bucket, and re-answering a bucket replaces that row rather than adding to the mean. The words are **Relaxed · Okay · Tense · Strained · Swamped** (`PsychStress.levels`): "Calm" is deliberately not one of them, because `StressBand.calm` already prints that word on the computed tile directly above the row. Tags and the note are report-only and reach no term.
