...
- ~~`MirrorPuller(windowDays:)` defaults **90**~~ — **corrected 2026-09-06 (Wave 13): the cap is gone in production.** At the time of this audit the initialiser defaulted to 90 days and ten `.window` tables (`daily_scores`, `nutrition_entries`, `water_intake`, `body_composition`, `sleep_sessions`, `cardio_logs`, `doms_logs`, `fatigue_logs`, `supplement_dose_overrides`, `daily_metrics`) could see neither older rows nor edits to older rows. §7.3 removed it: `MirrorPuller.since` returns `nil` for a `.window` table when `windowDays == nil`, and the one production wiring (`OnyxData/Sync/SyncCoordinator.swift:209-210`) passes `nil` to both `MirrorPuller` and `TrainingPuller`. The `= 90` on the two initialisers survives ONLY as a parameter default the package tests use to build a bounded fixture — no shipping call site takes it. Reading `= 90` and concluding the app still has a 90-day horizon is the misread this line exists to prevent.
...
> **Corrected 2026-09-06 (Wave 13).** The bullets above are the Phase 2 *diagnosis*, kept as the record of what was found. Phase 2 and Phase 2.5 closed every one of them, and a reader arriving at this section now should not treat it as current: `resetInFlight()` runs at the head of every coordinator cycle (`SyncCoordinator.swift:304`); realtime IS subscribed (`SyncCoordinator.swift:515`, `MirrorRealtime` behind `MirrorCoalescer`); pagination shipped in §7.3; `ScoringInputsBuilder` fills all three holes — `trailingAvgVolumeKg` (:228), `contextMode` (:314) and `newPRsToday` (:283). `swift:data` is 337 tests, `swift:core` 311. Two things named here are still genuinely open: `HKObserverQuery` background delivery is compiled behind `ONYX_ADP` (renamed from `HELIX_ADP` by Wave 2) and is NOT in the shipping configuration, and `BGAppRefreshTask` is not done. Finally, the directories this section names — `HelixNative/`, `HelixNativeWidgets/`, `HelixNativeTests/` — were renamed by Phase 2.5's Wave 2 and are now `native/Onyx/`, `native/OnyxWidgets/` and `native/OnyxTests/`; the same rename moved `HelixCore`/`HelixData`/`HelixUI` to `OnyxCore`/`OnyxData`/`OnyxUI`, which this document still spells the old way throughout §7 and §9 for the same historical reason.
...
> **Shipped 2026-09-05 (Wave 2.10).** `sleepQualityParts` / `computeMorningCharge(q, onsetTrouble)` and `stressParts` / `stressDrain` on both sides (`battery.ts` ↔ `Battery.swift`), `WORKOUT_MAX_BY_DAY` legs 32 / upper 24 / arms 16, `MAX_TOTAL_DRAIN = 89` asserted as `89 < 100` in `scoring.test.ts` and `InvariantTests`. Two new `ScoringInputs`: `sleepOnsetTrouble` (from `daily_logs`) and `fatigueLevel` (the LATEST slot of `fatigue_logs`, folded by the tracker's own rule — its pure half moved to `src/lib/recovery/fatigue.ts` so the server scorer can import it). Departures: the stages term reads `(deep + REM) / asleep` against a 45 % share (v7's "75 deep minutes = 1" is gone); every missing input degrades to its NEUTRAL value (HRV 0.5, RHR 1, stages 0 on a night with no minutes) so a night with nothing synced starts at 65, not 55; the onset penalty is applied after rounding and outside the clamp (worst start 52). `trailingAvgVolumeKg` was already wired in `ScoringInputsBuilder` (last six same-day-key sessions, maintenance excluded) — nothing to do. Vectors: `sleep-quality` (now with the four parts and the onset charge), `stress-drain` (new), `battery`, `battery-constants`, `workout-max`, `workout-drain`. `scripts/recompute-scores.mjs` is back (deleted in `4467c74`) and now walks every `daily_scores` row rather than a sleep-filtered subset; it needs a local `next start` of the new build, since the deployed route still runs v7.
...
> **Shipped 2026-09-05 (Wave 2.7).** `HelixCore/Charts/Series.swift` carries `E1rmSeries` (the per-session MEAN headline, pairs collapsed, `byReps` for unloaded work — the pure core lifted out of `useSessionTrends`, which now calls it), `SessionVolumeSeries` (bucketed by `VolumeSplit.resolve` on the session's own `day_key`), `MacroAdherenceSeries` (five verdicts: untracked / ungraded / exception / hit / miss; hit = kcal within ±10 % and protein ≥ 90 % of target) and `VitalSeries` (padded window, latest, delta vs the last differing reading, mean, coverage). "dailySeries padded to limit" is `WidgetDerive.paddedWindow` from 2.2, which now has its TS twin and a vector. `HelixCore/Training/ProgressionQueue.swift` is the pure queue (`src/lib/training/progressionQueue.ts`, lifted out of `useProgressionQueue`); `HelixData`'s `AppDatabase.progressionQueue(dayKey:program:phase:today:)` reads the ledger scoped to that key and era, and `WorkoutWeek` calls it instead of its own whole-history scan (which graded Legs A against Legs B sets). Vectors: `e1rm-series`, `sets-at-ceiling`, `session-volume-series`, `macro-adherence-series`, `vital-series`, `padded-window`, `progression-queue`. Departure: there is no `VitalSeries(metric)` enum — the metric is the caller's column pick, as `PulseModel` already does it; the series takes rows and a `Combine`.
...
> **Shipped 2026-09-05 (Wave 2.10).** `### Battery (v8 — the inputs behind the number the app showed)` under the existing `## Derived` fence, one line per day: the stored `daily_scores.battery_pct`, the wake charge with its four `q` terms and the onset flag, and the stress drain with its three terms and the latest fatigue word; `— (no score row)` for a day the app never scored. `ExportDay` gained three optional fields for it (`restingHrBaseline`, `hrvBaseline`, `batteryPct`) filled by both builders with the scorer's own window (the eight most recent logged days up to the date, minus the date). The raw body above the fence still prints no battery, score or active energy — `weekly-loop.test.ts` now asserts that on the body only. Departure: the six "reconstructed" session fields are NOT read from the mirror — `workout_sessions.total_volume_kg` / `set_count` / `avg_bpm` / `calories_burned` and `workout_sets.is_pr` are still not mirrored columns, so the builder keeps recomputing them from the sets (vector-proven against the web's own output). That read waits on the mirror carrying the columns.
...
> **Shipped 2026-09-05 (Wave 2.7).** `HelixAtlasHit.swift` and `onPick` arrived with Track U's 2.9 (reverse walk of `HelixAtlas.muscles`, `Path.contains`); 2.7 adds the shading (one 145° light across the figure — flesh, belly and worked fills lit from the same corner), the 0.5 pt hairline on every outline, a drop shadow under the silhouette's own layer, and `accessibilityChildren`: one element per landmark sized to its paths' bounds, label = the muscle, value = `values[muscle]` (the DOMS tile hands over "Mild / Moderate / Severe") or the worked share, button trait + action when `onPick` is set; a decorative figure stays hidden. The flip is 2.9's `DomsTile` (`rotation3DEffect` + `HelixMotion.move`). The 16 × 2 hit-test table lives in `HelixNativeTests/AtlasHitTests.swift` — `HelixUITests` cannot build for macOS (UIKit, `MeshGradient`) and never runs; the table probes each path's own interior on an 8 × 8 grid and asserts the answer is the last painted path and that every drawn landmark survives the z-order.
