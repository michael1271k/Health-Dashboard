...
private struct CaloriesCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: HelixSpace.s) {
            figures
            TargetBar(value: eaten, target: target, tint: HelixDomain.fuel.start, over: HelixDomain.fuel.end)
    ...
    private func hero(oneLine: Bool) -> some View {
        ...
        if oneLine {
            HStack(alignment: .firstTextBaseline, spacing: HelixSpace.xs) {
                Text(eaten.map(NutritionFormat.whole) ?? "—").helixHero()
                Text(against)
                    .helixType(.secondary)
                    .helixNumeral()
    ...
    private func remaining(oneLine: Bool) -> some View {
        if let left = model.remaining(eaten, target) {
            Text(NutritionFormat.remaining(left, unit: "kcal"))
                .helixType(.secondary)
                .helixNumeral()
                .foregroundStyle(left >= 0 ? Color.helix.textSecondary : HelixDomain.fuel.end)
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ZStack(alignment: .leading) {
                ...
                Capsule().fill(Color.helix.textTertiary.opacity(0.35))
                Capsule()
                    .fill(tint)
...
private struct AdherenceDots: View {
    ...
    private func colour(_ day: NutritionDay) -> Color {
        if ExceptionDay.isException(day.exception) { return HelixDomain.recover.accent }
        if day.estimated { return HelixDomain.fuel.start }
        return day.isTracked ? Color.helix.good : Color.helix.textTertiary
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            MacroRow(label: "Protein", eaten: model.eaten?.protein, target: model.target.protein,
                     tint: Color.helix.protein, model: model, onEdit: onEdit)
            divider
            MacroRow(label: "Carbs", eaten: model.eaten?.carbs, target: model.target.carbs,
                     tint: Color.helix.carbs, tracked: model.tracksCarbs, model: model, onEdit: onEdit)
            divider
            MacroRow(label: "Fat", eaten: model.eaten?.fat, target: model.target.fat,
                     tint: Color.helix.fat, tracked: model.tracksFat, model: model, onEdit: onEdit)
        }
...
private struct MacroRow: View {
    ...
    private var bar: some View {
        if tracked {
            TargetBar(value: eaten, target: target, tint: tint, over: Color.helix.danger)
                .frame(minWidth: 40)
        } else {
            Spacer(minLength: HelixSpace.s)
        }
    ...
    private var remaining: some View {
        if tracked, let left = model.remaining(eaten, target) {
            Text(NutritionFormat.remaining(left, unit: "g"))
                .helixType(.caption)
                .helixNumeral()
                .foregroundStyle(left >= 0 ? Color.helix.textTertiary : Color.helix.danger)
                .lineLimit(1)
                .fixedSize()
        }
    }
    ...
    var body: some View {
        HStack(spacing: HelixSpace.s) {
            Image(systemName: "drop.fill")
                .foregroundStyle(Color.helix.water)
                .accessibilityHidden(true)
            Text(figures)
                .helixType(.secondary)
                .helixNumeral()
            ...
            TargetBar(value: model.waterMl, target: model.waterGoalMl,
                      tint: Color.helix.water, over: Color.helix.water)
                .frame(minWidth: 40)
...
struct NutritionChip: View {
    ...
    var body: some View {
        Label(text, systemImage: icon)
            .helixType(.caption)
            .lineLimit(1)
            .foregroundStyle(style == .accent ? Color.helix.accent(.fuel) : Color.helix.textSecondary)
            .padding(.horizontal, HelixSpace.s)
            .frame(minHeight: 28)
            .background(Capsule().fill(Color.helix.hairline))
...
private struct WeekStrip: View {
    ...
    private var bars: [Bar] {
        let goals: [(String, Double?, (NutritionDay) -> Double?, Color)] = [
            ("Kcal", model.targetKcal, { $0.kcal }, HelixDomain.fuel.accent),
            ("P", model.target.protein, { $0.proteinG }, Color.helix.protein),
            ("C", model.tracksCarbs ? model.target.carbs : nil, { $0.carbsG }, Color.helix.carbs),
            ("F", model.tracksFat ? model.target.fat : nil, { $0.fatG }, Color.helix.fat),
        ]
...
struct NutritionBadge: View {
    ...
    var body: some View {
        Text(text)
            ...
            .padding(.horizontal, HelixSpace.xs)
            .padding(.vertical, 2)
