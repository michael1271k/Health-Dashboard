...
enum VitalGroup: String, CaseIterable, Identifiable {
    ...
    var domain: HelixDomain { self == .recovery ? .recover : .body }
...
struct VitalGroupCard: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                HelixSectionHeader(group.title, group.domain)
                Spacer(minLength: 8)
            ...
            HelixChartCard(
                ...
                if series.isEmpty {
                    HelixChartEmpty("No \(metric.label.lowercased()) from Apple Health yet.")
                } else {
                    chart(series)
                }
            }
    ...
    private func chart(_ series: [TrendPoint]) -> some View {
        Chart {
            ForEach(Trend.runs(series), id: \.point.d) { run in
                if let date = HelixChart.date(run.point.d) {
            ...
            if let selected, let hit = Trend.nearest(series, to: selected) {
                RuleMark(x: .value("Day", hit.date, unit: .day))
                    ...
                    .annotation(position: .top, overflowResolution: .init(x: .fit(to: .chart), y: .disabled)) {
                        HelixCallout(HelixChart.shortDate(hit.date), value: metric.format(hit.point.v))
                    }
            }
        }
