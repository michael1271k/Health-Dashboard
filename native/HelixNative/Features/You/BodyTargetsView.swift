...
struct BodyTargetsView: View {
    ...
    var body: some View {
        Form {
            Section {
                HelixNumberRow(label: "Weight", value: $draft.weight, field: Field.weight,
                               focus: $focus, unit: "kg", range: 0...300, fractionLength: 1,
                               onCommit: commit)
                HelixNumberRow(label: "Body fat", value: $draft.bodyFat, field: Field.bodyFat,
                               focus: $focus, unit: "%", range: 0...70, fractionLength: 1,
                               onCommit: commit)
                HelixNumberRow(label: "Skeletal muscle", value: $draft.muscle, field: Field.muscle,
                               focus: $focus, unit: "kg", range: 0...100, fractionLength: 1,
                               onCommit: commit)
            } header: {
                HelixSectionHeader("Destination", .body)
            } footer: {
