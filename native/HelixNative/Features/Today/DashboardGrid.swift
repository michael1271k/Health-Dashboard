...
struct DashboardGrid: View {
    ...
    var body: some View {
        ...
        .animation(reduceMotion ? .easeInOut(duration: 0.2) : HelixMotion.move, value: model.layout)
    ...
    private func tile(_ slot: StackSlot) -> some View {
        ...
        TileFrame(
            ...
            onEdit: { withAnimation(HelixMotion.flick) { model.editing = true } },
            ...
            if slot.items.count > 1 {
                ...
                HelixTile.face(slot.items[0], entry: model.entry)
            }
        }
