...
struct WeekDaysView: View {
    ...
    var body: some View {
            ...
            if let detail {
                Section {
                    WeekVitalsRow(vitals: detail.vitals)
                        .listRowInsets(.init(top: HelixSpace.s, leading: HelixSpace.l,
                                             bottom: HelixSpace.s, trailing: HelixSpace.l))
                }
        ...
        .toolbar {
            ToolbarItem(placement: .principal) {
                VStack(spacing: 0) {
                    Text(window.label).helixType(.body).foregroundStyle(Color.helix.textPrimary)
                    Text(window.rangeLabel).helixType(.micro).foregroundStyle(Color.helix.textSecondary)
                }
    ...
    private func actions(_ detail: HistoryWeeks.WeekDetail) -> some View {
        Section {
            ...
            HelixSectionHeader("This week", .recover)
        } footer: {
...
struct WeekVitalsRow: View {
    ...
    var body: some View {
        Group {
            ...
            if typeSize.isAccessibilitySize {
                ...
                LazyVGrid(
                    columns: Array(repeating: GridItem(.flexible(), spacing: HelixSpace.s), count: 4),
                    spacing: HelixSpace.m
                ) {
                    ForEach(entries) { entry in
    ...
    private func value(_ entry: Entry) -> some View {
        Text(entry.value ?? "—")
            .helixType(.secondary)
            .helixNumeral()
            .foregroundStyle(entry.value == nil ? Color.helix.textTertiary : entry.domain.accent)
            .lineLimit(1)
...
struct DayHistoryRow: View {
    ...
    var body: some View {
        HStack(spacing: HelixSpace.m) {
            ...
            Group {
                if day.isLogged {
                    Circle().fill(Color.helix.day(day.dayKey))
                } else if day.dayKey == nil {
                    Circle()
                        .fill(Color.helix.textTertiary.opacity(0.5))
                        .frame(width: 4, height: 4)
                } else {
                    Circle().strokeBorder(Color.helix.day(day.dayKey).opacity(day.isFuture ? 0.35 : 0.8), lineWidth: 1.5)
                }
            }
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(SessionRow.date(day.date))
                    .foregroundStyle(day.isFuture ? Color.helix.textTertiary : Color.helix.textPrimary)
                Text(title)
                    .helixType(.caption)
