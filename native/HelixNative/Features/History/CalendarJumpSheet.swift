    ...
    var body: some View {
        NavigationStack {
            ScrollViewReader { scroller in
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: HelixSpace.xl, pinnedViews: .sectionHeaders) {
...
private struct MonthGrid: View {
    ...
    private static let columns = Array(
        repeating: GridItem(.flexible(), spacing: HelixSpace.xs), count: 7
    )

    var body: some View {
        LazyVGrid(columns: Self.columns, spacing: HelixSpace.xs) {
                ...
                Button { onPick(cell.date) } label: {
                    VStack(spacing: 2) {
                        Text(String(Int(cell.date.suffix(2)) ?? 0))
                            .helixType(.caption)
                            .helixNumeral()
                            .foregroundStyle(cell.isFuture ? Color.helix.textTertiary : Color.helix.textPrimary)
                        Circle()
                            .fill(cell.isLogged ? Color.helix.day(cell.dayKey) : Color.clear)
