...
struct ExerciseDetailView: View {
    ...
    var body: some View {
        List {
            Section {
                ...
                .listRowInsets(EdgeInsets(top: 0, leading: HelixSpace.l, bottom: HelixSpace.xs, trailing: HelixSpace.l))
            }
    ...
    private var summary: some View {
        Section {
            VStack(alignment: .leading, spacing: HelixSpace.s) {
                HStack(spacing: HelixSpace.grid) {
                    stat("Heaviest", heaviest.map { "\(HelixFormat.kg($0)) kg" } ?? "—")
                    stat(timed ? "Longest hold" : "Best 1RM", bestReading)
                    stat("Best session", bestSessionVolume.map { "\(HelixFormat.volume($0)) kg" } ?? "—")
                }
            ...
            HelixSectionHeader("Best", group.domain)
        }
        ...
        Section {
            HelixChartCard(
                ...
                if !loaded {
                    HelixChartEmpty("Reading the ledger…")
                    ...
                    HelixChartEmpty("Two sessions and the line starts.")
                }
            }
        ...
        ForEach(sessions, id: \.id) { session in
            Section {
                ...
                HStack(alignment: .firstTextBaseline) {
                    HelixSectionHeader(session.title, group.domain)
                    Spacer()
                    Text(session.meta)
    ...
    private func chip(_ set: HistorySetRow) -> some View {
        ...
        return Text(SetFormat.format(weightKg: set.weightKg, reps: Double(set.reps), timed: timed))
            .helixType(.caption).helixNumeral()
            .foregroundStyle(work ? Color.helix.textPrimary : Color.helix.textTertiary)
            .padding(.horizontal, HelixSpace.s)
            .padding(.vertical, HelixSpace.xs)
            .background(
                work ? group.domain.accent.opacity(0.14) : Color.helix.hairline,
                in: RoundedRectangle(cornerRadius: HelixCorner.row, style: .continuous)
            )
    }
    ...
    private var sessions: [SessionBlock] {
        ...
        return order.reversed().map { id in
            ...
            if volume > 0 { meta += " · \(HelixFormat.volume(volume)) kg" }
