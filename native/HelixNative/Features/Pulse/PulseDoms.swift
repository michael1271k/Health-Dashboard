...
struct DomsTile: View {
    ...
    private var colors: [LandmarkMuscle: Color] {
        ...
        for (group, level) in severity where level > 0 {
            for landmark in DomsMap.landmarks[group] ?? [] {
                out[landmark] = Color.helix.severity(level)
            }
        }
        return out
    }
    ...
    var body: some View {
        DayTile("Soreness", .recover) {
            ...
            Text(showingBack ? "Back" : "Front").helixMicro()
        }
    ...
    private func side(_ view: HelixAtlasView) -> some View {
        ...
        // `HelixAtlas.muscles` rather than this dictionary — so the whole body
    ...
    private func flip() {
        withAnimation(reduceMotion ? HelixMotion.fade : HelixMotion.move) { showingBack.toggle() }
    }
    ...
    private var sideDots: some View {
        HStack(spacing: HelixSpace.xs) {
            ForEach([false, true], id: \.self) { back in
                Circle()
                    .fill(showingBack == back ? Color.helix.accent(.recover) : Color.helix.hairline)
