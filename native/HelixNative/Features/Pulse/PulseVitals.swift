...
struct VitalRows: View {
    ...
    var body: some View {
        ForEach(VitalSpec.all, id: \.name) { spec in
            ...
            MetricRow(
                name: spec.name,
                value: HelixSnapshot.fixed(vital?.value, decimals: spec.decimals),
                unit: spec.unit,
                delta: vital?.delta,
        ...
        activity("Steps", window.steps, unit: "steps", grouped: true, upIsGood: true, color: HelixDomain.body.accent)
        activity("Stand hours", window.standHours, unit: "h", upIsGood: true, color: HelixDomain.body.at(0.5))
        activity("Active energy", window.activeKcal, unit: "kcal", grouped: true, upIsGood: true, color: HelixDomain.fuel.accent)
    }
    ...
    private func activity(
        ...
        MetricRow(
            name: name,
            value: block?.value.map { grouped ? NutritionFormat.whole($0) : HelixSnapshot.fixed($0, decimals: 0) ?? "—" },
            unit: unit,
