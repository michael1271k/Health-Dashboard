...
struct SleepTile: View {
    ...
    private var segments: [(HelixSleepStage, Int)] {
        guard let night else { return [] }
        return [
            (HelixSleepStage.deep, night.deepMin),
            (.rem, night.remMin),
            (.core, night.coreMin),
    ...
    private var arc: some View {
        VStack(spacing: HelixSpace.s) {
            ...
            if let window { Text(window).helixType(.caption).helixNumeral().foregroundStyle(Color.helix.textSecondary) }
            goalChip
        }
    ...
    private var stageGrid: some View {
        LazyVGrid(
            columns: Array(
                repeating: GridItem(.flexible(), spacing: HelixSpace.m),
    ...
    private func bank(_ debt: SleepDebt) -> some View {
        VStack(alignment: .leading, spacing: HelixSpace.xs) {
            Gauge(value: min(debt.debtHours, 10), in: 0...10) {
                Text("Sleep debt · 14-night").helixMicro()
            } currentValueLabel: {
    ...
    private var onsetRow: some View {
        Toggle(isOn: Binding(
            ...
            Text("Trouble falling asleep").helixType(.body)
        }
        .tint(accent)
