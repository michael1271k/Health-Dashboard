...
struct FatigueSummaryRow: View {
    ...
    var body: some View {
        PulseRow(
            ...
            title: "Fatigue",
            detail: detail,
            tint: Color.helix.fatigue(latest?.level),
            ...
            HStack(spacing: HelixSpace.s) {
                if let level = latest?.level, let word = Fatigue.level(level)?.label {
                    Text(word)
                        ...
                        .foregroundStyle(Color.helix.fatigue(level))
                        .lineLimit(1)
                }
                cost
                dots
    ...
    private var dots: some View {
        HStack(spacing: HelixSpace.xs) {
            ForEach(slots, id: \.self) { slot in
                Circle()
                    .fill(day[slot] != nil ? Color.helix.fatigue(day[slot]) : .clear)
                    .strokeBorder(day[slot] != nil ? .clear : Color.helix.textTertiary, lineWidth: 1)
    ...
    private var cost: some View {
        if let delta = Fatigue.delta(day) {
            Text("\(delta >= 0 ? "+" : "")\(delta)")
                ...
                .foregroundStyle(delta > 1 ? Color.helix.record : Color.helix.textSecondary)
...
struct FatigueSheet: View {
            ...
            Form {
                ForEach(model.fatigueSlots, id: \.self) { slot in
                    Section {
                        ...
                        HelixSectionHeader(slot.label, .recover)
                    }
                }
                if model.isTraining {
                    Section {
    ...
    private func picker(_ slot: FatigueSlot) -> some View {
        ...
        .pickerStyle(.inline)
        .labelsHidden()
        .tint(Color.helix.accent(.recover))
    }
}
