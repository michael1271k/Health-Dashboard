                ...
                Button {
                    ...
                    HStack(alignment: .firstTextBaseline, spacing: 12) {
                            ...
                            HStack(spacing: 6) {
                                ...
                                if plan.isLegacy {
                                    Text("legacy")
                                        ...
                                        .padding(.vertical, 2)
                                        .background(Color.onyx.hairline, in: .capsule)
                        ...
                        Image(systemName: plan.id == selectedPlanId ? "checkmark.circle.fill" : "circle")
                            .foregroundStyle(plan.id == selectedPlanId ? OnyxDomain.train.accent : Color.onyx.textTertiary)
    ...
    private var consequences: some View {
        ...
        Section {
            ...
            LabeledContent("Protein · carbs · fat") {
                Text(macroLine(goals)).onyxNumeral()
            }
            ...
            OnyxSectionHeader(isActive ? "In force" : "What changes", .fuel)
        }
    }
    ...
    private var routine: some View {
        Section {
            if let program = model.deck(for: selectedPlanId) {
                ...
                // Only ONYX-5's deck is ported. Saying so is the honest answer;
            ...
            OnyxSectionHeader("What you do each day", .train)
        } footer: {
    ...
    var body: some View {
        Form {
            Section {
                ...
                OnyxSectionHeader(day.sub ?? day.label, .train)
            } footer: {
