...
struct LeversView: View {
        ...
        return Button {
            ...
            HStack(alignment: .firstTextBaseline, spacing: 12) {
                ...
                Image(systemName: isOn ? "checkmark.circle.fill" : "circle")
                    .foregroundStyle(isOn ? OnyxDomain.fuel.accent : Color.onyx.textTertiary)
    ...
    private var release: some View {
        Section {
            ...
            OnyxSectionHeader("Release", .fuel)
        } footer: {
    ...
    private var targets: some View {
        Section {
            if let held = model.heldBy {
                OnyxHeldRow(label: "Calories", value: held.calorieGoal, unit: "kcal")
                OnyxHeldRow(label: "Protein", value: held.proteinGoalG, unit: "g")
                OnyxHeldRow(label: "Carbohydrate", value: held.carbsGoalG, unit: "g")
                OnyxHeldRow(label: "Fat", value: held.fatGoalG, unit: "g")
                OnyxHeldRow(label: "Steps", value: held.stepsGoal, unit: "steps")
            } else {
                OnyxNumberRow(label: "Calories", value: $draft.kcal, field: Field.kcal,
                               focus: $focus, unit: "kcal", range: 0...8000, onCommit: commitGoals)
                OnyxNumberRow(label: "Protein", value: $draft.protein, field: Field.protein,
                               focus: $focus, unit: "g", range: 0...600, onCommit: commitGoals)
                OnyxNumberRow(label: "Carbohydrate", value: $draft.carbs, field: Field.carbs,
                               focus: $focus, unit: "g", range: 0...900, onCommit: commitGoals)
                OnyxNumberRow(label: "Fat", value: $draft.fat, field: Field.fat,
                               focus: $focus, unit: "g", range: 0...400, onCommit: commitGoals)
                OnyxNumberRow(label: "Steps", value: $draft.steps, field: Field.steps,
                               focus: $focus, unit: "steps", range: 0...60000, onCommit: commitGoals)
            }
        } header: {
            OnyxSectionHeader(model.heldBy == nil ? "Targets" : "Targets · held", .fuel)
        } footer: {
            targetsFooter
        }
    }
    ...
    private var recovery: some View {
        Section {
            OnyxNumberRow(label: "Active energy", value: $draft.activeCal, field: Field.activeCal,
                           focus: $focus, unit: "kcal", range: 0...3000, onCommit: commitRecovery)
            OnyxNumberRow(label: "Sleep", value: $draft.sleep, field: Field.sleep,
                           focus: $focus, unit: "h", range: 0...16, fractionLength: 2,
                           onCommit: commitRecovery)
            OnyxNumberRow(label: "Water", value: $draft.water, field: Field.water,
                           focus: $focus, unit: "ml", range: 0...8000, onCommit: commitRecovery)
        } header: {
            OnyxSectionHeader("Recovery & activity", .recover)
        } footer: {
