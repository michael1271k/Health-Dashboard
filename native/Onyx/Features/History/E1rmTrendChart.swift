...
/// date snaps to it, and a `RuleMark` with a `OnyxCallout` is the tooltip.
...
struct E1rmTrendChart: View {
    ...
    private var dates: [Date] {
        Array(Set(series.flatMap { $0.points.compactMap { OnyxChart.date($0.date) } })).sorted()
    }
    ...
    var body: some View {
        if let scrollDays {
            chart.onyxScrollable(days: scrollDays, endingAt: dates.last ?? Date())
        } else {
            chart
        }
    }
        ...
        Chart {
            ForEach(series) { s in
                ForEach(s.points, id: \.date) { p in
                    if let date = OnyxChart.date(p.date) {
            ...
            if let picked = nearest(selected) {
                RuleMark(x: .value("Selected", picked))
                    ...
                    .annotation(position: .top, spacing: 0, overflowResolution: .init(x: .fit(to: .chart), y: .disabled)) {
                        OnyxCallout(OnyxChart.shortDate(picked), lines: lines(at: picked))
                    }
            }
        }
        .chartForegroundStyleScale(domain: series.map(\.id), range: series.indices.map { Color.onyx.series($0) })
    ...
    private func lines(at date: Date) -> [OnyxCallout.Line] {
        series.enumerated().compactMap { i, s in
            guard let p = s.points.first(where: { OnyxChart.date($0.date) == date }) else { return nil }
            return OnyxCallout.Line(series.count > 1 ? s.id : "", "\(jsIntegerString(jsRound1(p.kg))) kg", color: series.count > 1 ? Color.onyx.series(i) : nil)
        }
    }
}
