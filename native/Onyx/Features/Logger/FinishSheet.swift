...
struct FinishSheet: View {
    ...
    private var summary: some View {
        VStack(spacing: 0) {
            ...
            divider
            line("Tonnage", "\(OnyxFormat.volume(model.totalVolumeKg)) kg")
            divider
            line("Duration", "\(minutes) min")
            if model.recordCount > 0 {
                divider
                line("Records", "\(model.recordCount)", tint: Color.onyx.record)
            }
    ...
    private func line(_ label: String, _ value: String, tint: Color = Color.onyx.textPrimary) -> some View {
...
private struct RPEDial: View {
    ...
    var body: some View {
        ...
        .accessibilityValue(rated ? "RPE \(OnyxFormat.rpe(value))" : "Not rated")
