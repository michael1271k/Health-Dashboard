...
/// reviewed by reading it, which is how it kept a `Text("HELIX")` through a
...
/// `HelixWorkoutAttributes` (see its header), so the app's preview harness can
...
struct WorkoutLockCard: View {
    ...
    private var accent: Color { Color.helix.day(state.dayKey) }
...
/// lesson of "Helix 1/2 75x13": before watchOS 11 the Smart Stack mirrored the
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 1) {
            HStack(spacing: 6) {
                ...
                Text(state.exercise)
                    .font(HelixWidgetType.label(12))
                    .foregroundStyle(.white)
                    .lineLimit(1)
                    .minimumScaleFactor(0.8)
                if !state.setLabel.isEmpty {
                    Text(state.setLabel)
                        .font(HelixWidgetType.label(9, weight: .bold))
                        .tracking(0.6)
            ...
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                if !state.load.isEmpty {
                    Text(state.load)
                        .font(HelixWidgetType.figure(17))
                        .foregroundStyle(.white)
                }
    ...
    var body: some View {
        Button(intent: RestSkipIntent()) {
            Label("Skip rest", systemImage: "forward.fill")
                ...
                .foregroundStyle(Color.helix.day(dayKey))
                .padding(.horizontal, 14)
                .frame(minHeight: 44)
                .background(Color.helix.day(dayKey).opacity(0.16), in: Capsule())
        }
        .buttonStyle(.plain)
