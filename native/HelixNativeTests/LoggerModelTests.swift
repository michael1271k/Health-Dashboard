...
/// `HelixCore` and `HelixData` test on the command line in seconds, which is
...
struct LoggerModelTests {
    ...
    private func armsBulk() -> LoggerModel {
        LoggerModel(day: Program.helix5.day(key: "arms")!, phase: .bulk)
    }
    ...
    func bodyweightSetsCount() {
        let model = LoggerModel(day: Program.helix5.day(key: "legs_b")!, phase: .cut)
