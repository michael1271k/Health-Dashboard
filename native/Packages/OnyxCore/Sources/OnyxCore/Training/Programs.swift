...
/// prescription. Only Onyx-5's deck is ported (`Program.onyx5`), and the
...
public extension PhaseGoals {
    ...
    /// The Onyx cut. 1955 kcal = 170·4 + 195·4 + 55·9, exactly.
    ///
    ...
    /// The Onyx lean bulk. `fatGoalG` is a HARD CAP rather than a target.
    ///
    /// Note that 160·4 + 330·4 + 70·9 is 2590, not 2600: this triple is the one
    /// place in ONYX where the calorie figure is NOT the Atwater sum of its
    ...
    /// `NUTRITION_PRESETS[mode]` — the Onyx defaults, before any plan overlay.
...
public extension Programs {
    ...
    /// `PLAN_PHASES` — per-plan overrides layered on the Onyx defaults.
    ///
    ...
    private static let planOverrides: [String: [ProgramPhase: PhaseGoals]] = [
        ...
        // 55·9. It inherits everything else, INCLUDING the Onyx cut's body
