...
public extension AppDatabase {
    ...
    static func enqueueRowUpsert(table: String, id: String, nulls: [String] = [], in db: Database) throws {
        ...
        if let previous = try OutboxItem.filter(Column("idempotency_key") == key).fetchOne(db),
           let ref = try? OnyxJSON.decoder.decode(RowRef.self, from: previous.payload) {
        ...
        var item = OutboxItem(
            ...
            payload: try OnyxJSON.encoder.encode(RowRef(table: table, id: id, nulls: cleared.sorted())),
            idempotencyKey: key
        )
        try item.insert(db)
    }
    ...
    static func enqueueRowDelete(table: String, key: [String: String], in db: Database) throws {
        ...
        var item = OutboxItem(
            ...
            payload: try OnyxJSON.encoder.encode(RowDeleteRef(table: table, key: key)),
