...
// face the family was asked to lead with. This used to be `HelixEntry:
...
// The old entry carried `status: HelixSnapshotClient.Status` — whether the last
...
/// to render something.
public enum HelixFocus: Sendable, Equatable {
  case fuel(FuelFocus)
...
public struct HelixTileEntry: Sendable {
  public let date: Date
  public let snapshot: HelixSnapshot?
  public var focus: HelixFocus?

  public init(date: Date = Date(), snapshot: HelixSnapshot?, focus: HelixFocus? = nil) {
    self.date = date
  ...
  public static func placeholder(_ date: Date = Date()) -> HelixTileEntry {
    HelixTileEntry(date: date, snapshot: nil, focus: nil)
  }
}
...
// picker titles) in its own `HelixIntents.swift` — a tile drawn inside the app
...
public enum HelixLink {
  /// The destination is a PATH, carried as a query parameter rather than a
  /// host — `helix://nutrition` would make "nutrition" a host and lose
