...
/// The sixteen muscles HELIX actually tracks, and the one rule for crediting a
/// set to them.
///
...
/// `MuscleDistribution.tsx`. The arithmetic is in `HelixCore` rather than in a
...
public enum LandmarkMuscle: String, CaseIterable, Codable, Sendable, Hashable {
    ...
    ///
    /// `HelixAtlas.muscles` keys on these strings, so this is not merely a
...
public enum MuscleCredit {
    ...
    /// Nowhere else in HELIX does a warm-up count as a set. Here it does,
