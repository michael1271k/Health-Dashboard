...
// heaviest thing Helix has seen" the all-time best and flags a return to an old
...
// everything set inside the window Helix can see, so fed straight to
...
// therefore asserts only the EXCESS of the book over `logged`, what Helix's own
...
public enum PrTruth {
    ...
    /// What Helix's OWN complete set history produces — the subtrahend.
    ...
    /// The effective floor for one exercise — the part of the book Helix's own
    ...
    /// asserted best SET (Helix's own arithmetic, directly comparable to what
    /// detection produces) and Hevy's figure — but the latter only where the
    /// max weight also floors, i.e. where the set behind it is one Helix never
