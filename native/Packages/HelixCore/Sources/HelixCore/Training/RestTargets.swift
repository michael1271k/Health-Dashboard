...
//
// Helix used to MEASURE rest (the gap between two set ticks) and never
...
// belong to HelixData; the keys they are stored under are defined here so the
...
public enum RestTargets {
    ...
    public static func programRestSec(for exerciseName: String, dayKey: String?, program: Program = .helix5, phase: ProgramPhase = .cut) -> Double? {
