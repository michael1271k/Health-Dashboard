...
struct TrainingTests {
    ...
    func cutDropsOnlyTheZeroes() {
        let arms = Program.helix5.day(key: "arms")!
        let legsB = Program.helix5.day(key: "legs_b")!
    ...
    func cutIsLighterEverywhere() {
        for day in Program.helix5.days {
    ...
    func nilIsNotZero() {
        let legsA = Program.helix5.day(key: "legs_a")!
    ...
    func theWeekIsIntact() {
        #expect(Program.helix5.days.map(\.weekday) == [0, 1, 2, 4, 5])
        // Wed (3) and Sat (6) are Zone-2 rest and have no deck at all.
        #expect(Program.helix5.day(weekday: 3) == nil)
        #expect(Program.helix5.day(weekday: 6) == nil)
    }
}
