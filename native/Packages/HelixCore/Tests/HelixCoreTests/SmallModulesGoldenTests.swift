...
private func resolver(_ overrides: [String: String]) -> (String, DayLayout) -> ScheduleDay? {
    { dateISO, layout in
        if let o = overrides[dateISO] {
            if o == ScheduleSwap.restOverride { return nil }
            if o.hasPrefix("label:") { return ScheduleDay(label: String(o.dropFirst(6))) }
            return dayOf(o)
        }
        guard let w = ScheduleSwap.weekday(dateISO), let key = ScheduleLayout.dayKey(for: w, program: helix5, layout: layout) else { return nil }
        return dayOf(key)
    }
}

@Suite("Item #10 — schedule layout")
struct ScheduleLayoutGoldenTests {
    struct Move: Decodable { let layout: [String: Int]; let dayKey: String; let weekday: Int }
    struct In: Decodable { let raw: JSONValue?; let rawEntries: [[JSONValue]]?; let move: Move? }
    struct Out: Decodable {
        let layout: [[JSONValue]]; let full: [[JSONValue]]; let byWeekday: [String?]; let effective: [Int]
        let authored: Bool; let canonical: String; let moved: [[JSONValue]]?
    }

    @Test("parse, full, move, authored and canonical match")
    func layout() throws {
        for c in try GoldenFixture<In, Out>.load("schedule-layout").cases {
            let layout: DayLayout
            let probe: DayLayout
            if let m = c.input.move {
                layout = DayLayout(m.layout.map { ($0.key, $0.value) })
                probe = ScheduleLayout.moveDay(helix5, layout, dayKey: m.dayKey, weekday: m.weekday)
                #expect(probe.pairs.map { "\($0.0)=\($0.1)" } == pairs(c.expected.moved!).map { "\($0.0)=\($0.1)" }, "moved — \(c.name)")
            } else {
                layout = ScheduleLayout.parse(c.input.rawEntries.map { $0.map { ($0[0].string!, $0[1].any) } })
                probe = layout
                #expect(c.expected.moved == nil)
            }
            #expect(layout.pairs.map { "\($0.0)=\($0.1)" } == pairs(c.expected.layout).map { "\($0.0)=\($0.1)" }, "layout — \(c.name)")
            #expect(ScheduleLayout.full(helix5, layout).pairs.map { "\($0.0)=\($0.1)" } == pairs(c.expected.full).map { "\($0.0)=\($0.1)" }, "full — \(c.name)")
            #expect((0..<7).map { ScheduleLayout.dayKey(for: $0, program: helix5, layout: probe) } == c.expected.byWeekday, "byWeekday — \(c.name)")
            #expect(helix5.days.map { ScheduleLayout.effectiveWeekday($0, probe) } == c.expected.effective, "effective — \(c.name)")
            #expect(ScheduleLayout.isAuthored(helix5, probe) == c.expected.authored, "authored — \(c.name)")
            #expect(ScheduleLayout.canonical(probe) == c.expected.canonical, "canonical — \(c.name)")
        }
    }
}
...
struct ScheduleSwapGoldenTests {
    ...
    struct Swap: Decodable { let dateISO: String; let dayKey: String; let naturalDate: String? }
    ...
    struct In: Decodable { let overrides: [String: String]; let layout: [String: Int]; let rest: Rest?; let swap: Swap?; let block: Block?; let permanent: Permanent? }
    ...
    func swaps() throws {
        ...
        for c in try GoldenFixture<In, Out>.load("schedule-swap").cases {
            let layout = DayLayout(c.input.layout.map { ($0.key, $0.value) })
            let resolveWith = resolver(c.input.overrides)
            let resolve: ResolveDay = { resolveWith($0, layout) }
            if let r = c.input.rest {
                let p = r.horizon.map { ScheduleSwap.planRestDay(r.dateISO, resolve: resolve, horizon: $0) } ?? ScheduleSwap.planRestDay(r.dateISO, resolve: resolve)
                ...
                #expect(ScheduleSwap.describe(p) == e.sentence, "rest sentence — \(c.name)")
            }
            if let s = c.input.swap {
                #expect(ScheduleSwap.planDaySwap(s.dateISO, dayKey: s.dayKey, resolve: resolve, naturalDate: s.naturalDate) == c.expected.swap, "swap — \(c.name)")
            }
            if let b = c.input.block {
                let block = ScheduleSwap.blockForPlacement(b.dateISO, dayKey: b.dayKey, logged: b.logged, sourceDate: b.sourceDate)
                #expect(block == c.expected.block!.block, "block — \(c.name)")
                #expect(block.map { ScheduleSwap.describe($0, labelFor: labelFor) } == c.expected.block!.sentence, "block sentence — \(c.name)")
            }
            if let p = c.input.permanent {
                let plan = ScheduleSwap.planPermanentMove(program: helix5, layout: layout, dayKey: p.dayKey, weekday: p.weekday, todayISO: p.todayISO, logged: p.logged, resolveWith: resolveWith)
                let e = c.expected.permanent!
                #expect(plan.layout?.values == e.layout, "permanent layout — \(c.name)")
    ...
    func week() throws {
        for c in try GoldenFixture<WeekIn, WeekOut>.load("schedule-week").cases {
            #expect(ScheduleSwap.weekDates(of: c.input.dateISO) == c.expected.week, "week — \(c.name)")
            #expect((0..<7).map { ScheduleSwap.date(forWeekday: $0, in: c.input.dateISO)! } == c.expected.forWeekday, "forWeekday — \(c.name)")
            #expect(ScheduleSwap.shortDayLabel(c.input.dateISO) == c.expected.label, "label — \(c.name)")
            #expect(c.expected.week.map(ScheduleSwap.shortDayLabel) == c.expected.weekLabels, "weekLabels — \(c.name)")
        }
    }
}
...
struct ExerciseGoldenTests {
    ...
    func flags() throws {
        ...
        for c in fixture.cases {
            ...
            if let name = n {
                #expect(MuscleMap.lookup(name) == c.expected.muscles, "muscles — \(c.name)")
                #expect(MuscleMap.muscleGroups(for: name) == c.expected.groups, "groups — \(c.name)")
    ...
    func dictionary() throws {
        let dict = try GoldenFixture<JSONValue?, [MuscleDictEntry]>.load("muscle-dict").cases[0].expected
...
struct SupplementsGoldenTests {
    struct In: Decodable { let isTraining: Bool; let weekday: Int; let dbSlots: [SupplementSlot] }
    struct Out: Decodable { let stack: [SupplementSlot]; let count: Int }

    @Test("the seed and the day resolver match")
    func supplements() throws {
        for c in try GoldenFixture<In, Out>.load("supplements").cases {
            let i = c.input
            #expect(Supplements.stackForDate(i.dbSlots, isTraining: i.isTraining, weekday: i.weekday) == c.expected.stack, "stack — \(c.name)")
            #expect(Supplements.count(isTraining: i.isTraining, weekday: i.weekday, dbSlots: i.dbSlots) == c.expected.count, "count — \(c.name)")
        }
        #expect(Supplements.allKeys == ["multivitamin", "d3k2", "citrulline", "caffeine", "creatine", "omega3", "magnesium", "glycine", "theanine"])
    }
