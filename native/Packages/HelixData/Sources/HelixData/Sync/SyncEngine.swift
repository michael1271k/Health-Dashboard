...
public actor SyncEngine {
    ...
    static func target(of item: OutboxItem) throws -> (item: OutboxItem, sessionId: String, setId: String?) {
        if item.kind.hasPrefix(SyncKind.setEventPrefix) {
            do {
                let event = try HelixJSON.decoder.decode(SetEvent.self, from: item.payload)
        ...
        if item.kind == SyncKind.sessionUpsert {
            do {
                let ref = try HelixJSON.decoder.decode(SessionRef.self, from: item.payload)
    ...
    /// rather than `Optional(HelixData.SyncError.unmappedDayKey("legs_c"))`.
