...
public struct WidgetSnapshotBuilder: Sendable {
    ...
    public func build(scope: HelixScope, now: Date = Date()) throws -> HelixSnapshot {
        ...
        let scores = HelixSnapshot.Scores(
        ...
        let context: HelixSnapshot.DayContext? = {
            ...
            return HelixSnapshot.DayContext(mode: mode.rawValue, label: meta.label)
        }()

        let streak = Streak.programDayCount(date)

        // ── The scoped slices ─────────────────────────────────────────────
        let stepsTrend: [HelixSnapshot.Point]? = wantsLifestyle ? Self.points(stepsTrend(rows, date: date)) : nil
        let vitals: HelixSnapshot.Vitals? = wantsLifestyle ? vitalsSlice(rows.logs, date: date) : nil
        let performance = wantsPerformance ? performanceSlice(rows, date: date, weekStart: weekStart) : nil
        let volumeTrend: [HelixSnapshot.Point]? = wantsTraining || wantsPerformance
            ...
            : nil
        let cardio: HelixSnapshot.Cardio? = wantsTraining ? cardioSlice(rows.cardio, date: date, weekStart: weekStart) : nil
        let body: HelixSnapshot.Body? = wantsBody ? bodySlice(rows.weights) : nil
        let readiness: HelixSnapshot.Readiness? = {
            ...
            return HelixSnapshot.Readiness(level: r.level.rawValue, label: r.label, color: r.color, reason: r.reason)
        }()
        ...
        return HelixSnapshot(
            ...
            week: {
                ...
                return HelixSnapshot.Week(sessions: t.sessions, volumeKg: t.volumeKg, prs: t.prs, sets: t.sets,
            ...
            today: todaySessions.isEmpty ? nil : HelixSnapshot.Today(
            ...
            streak: HelixSnapshot.Streak(current: streak, best: streak),
            context: context,
            cardio: cardio,
            calendar: wantsTraining ? calendarGrid.map {
                HelixSnapshot.CalendarDay(d: $0.d, dayKey: $0.dayKey, label: $0.label, scheduled: $0.scheduled, logged: $0.logged, volumeKg: $0.volumeKg)
            } : nil,
    ...
    struct LiveScore {
        let total: Int
        let battery: Int
        let components: HelixCore.ScoreComponents
    }
    ...
    func performanceSlice(_ rows: Rows, date: String, weekStart: String) -> (records: [HelixSnapshot.Record], e1rm: [HelixSnapshot.E1rm], volumeByFamily: [HelixSnapshot.FamilyVolume]) {
        ...
        return (
            WidgetDerive.topRecords(Array(ledger), limit: 6).map {
                HelixSnapshot.Record(exercise: $0.exercise, axis: $0.axis, value: $0.value, reps: $0.reps.map { Int($0) }, achievedOn: $0.achievedOn)
            },
            WidgetDerive.e1rmTrends(sets, asOf: date, limit: 5).map {
                HelixSnapshot.E1rm(exercise: $0.exercise, kg: $0.kg, deltaKg: $0.deltaKg, trend: Self.points($0.trend))
            },
            WidgetDerive.volumeByFamily(weekSets).map {
                HelixSnapshot.FamilyVolume(family: $0.family, kg: $0.kg, sets: $0.sets)
            }
        )
    }

    /// Five overnight readings, each against its own fortnight baseline.
    func vitalsSlice(_ logs: [DailyLogRow], date: String) -> HelixSnapshot.Vitals {
        func of(_ pick: (DailyLogRow) -> Double?) -> HelixSnapshot.Vital {
            let b = WidgetDerive.vitalBlock(logs.map { DatedValue(date: $0.date, value: pick($0)) }, todayISO: date, trendLimit: Self.trendDays)
            return HelixSnapshot.Vital(value: b.value, baseline: b.baseline, trend: Self.points(b.trend))
        }
        return HelixSnapshot.Vitals(
            hrvMs: of { $0.hrvMs },
    ...
    func cardioSlice(_ rows: [CardioLogRow], date: String, weekStart: String) -> HelixSnapshot.Cardio {
        ...
        return HelixSnapshot.Cardio(
            last: block.last.map {
                HelixSnapshot.Cardio.Session(kind: $0.kind, date: $0.date, distanceM: $0.distanceM, durationMin: $0.durationMin, paceMinPerKm: $0.paceMinPerKm)
            },
    ...
    func bodySlice(_ rows: [BodyCompositionRow]) -> HelixSnapshot.Body {
    ...
    static func points(_ trend: [TrendPoint]) -> [HelixSnapshot.Point] {
        trend.map { HelixSnapshot.Point(d: $0.d, v: $0.v) }
    }
