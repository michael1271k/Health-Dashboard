        ...
        try writer.write { db in
            ...
            if let level {
                var row = existing.first
                    ?? FatigueLogRow(id: newHelixID(), userId: userId, date: date, slot: slot, level: level, createdAt: now)
                row.level = level
                try row.save(db)
    ...
    func setDoms(
        ...
        try writer.write { db in
            var row = try DomsLogRow
                ...
                .fetchOne(db)
                ?? DomsLogRow(id: newHelixID(), userId: userId, date: date, muscleGroup: muscleGroup, severity: severity, createdAt: now)
    ...
    func setManualMacros(
        ...
        try writer.write { db in
            var row = try NutritionEntryRow
                ...
                .fetchOne(db)
                ?? NutritionEntryRow(
                    id: newHelixID(), userId: userId, loggedAt: Self.utcInstant(date, hour: 12) ?? now,
    ...
    func setWaterOverride(userId: String, date: String, ml: Double, now: Date = Date()) throws {
        try writer.write { db in
            ...
            var row = WaterIntakeRow(
                id: newHelixID(), userId: userId, hkUuid: "manual-water-\(date)",
