...
/// `HELIX_SNAPSHOT_URL` / `HELIX_SNAPSHOT_TOKEN`, so there is one convention for
...
public struct SupabaseConfig: Sendable {
    ...
    public enum ConfigError: Error, CustomStringConvertible {
        ...
        public var description: String {
            ...
            case .missing(let key):
                return """
                \(key) is not set. Copy native/HelixNative/Support/Secrets.example.xcconfig \
    ...
    public static func fromBundle(_ bundle: Bundle = .main) throws -> SupabaseConfig {
        guard let raw = bundle.object(forInfoDictionaryKey: "HELIX_SUPABASE_URL") as? String,
              !raw.isEmpty,
        ...
        else { throw ConfigError.missing("HELIX_SUPABASE_URL") }

        guard let key = bundle.object(forInfoDictionaryKey: "HELIX_SUPABASE_ANON_KEY") as? String,
              !key.isEmpty
        else { throw ConfigError.missing("HELIX_SUPABASE_ANON_KEY") }
