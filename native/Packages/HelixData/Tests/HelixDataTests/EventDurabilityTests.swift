...
struct EventDurabilityTests {
    ...
    func bodyWireShapeIsStable() throws {
        ...
        let json = String(decoding: try HelixJSON.encoder.encode(body), as: UTF8.self)
        ...
        #expect(try HelixJSON.decoder.decode(SetEvent.Body.self, from: Data(json.utf8)) == body)
    }

    @Test("a void body round-trips without a payload")
    func voidBodyRoundTrips() throws {
        let json = try HelixJSON.encoder.encode(SetEvent.Body.void)
        #expect(try HelixJSON.decoder.decode(SetEvent.Body.self, from: json) == .void)
    }
    ...
    func datesAreISO8601() throws {
        ...
        let json = String(decoding: try HelixJSON.encoder.encode(event), as: UTF8.self)
