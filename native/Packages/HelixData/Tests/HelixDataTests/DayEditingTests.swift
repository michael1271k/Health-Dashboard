...
struct DayEditingTests {
    ...
    private func deleteRefs(_ db: AppDatabase) throws -> [RowDeleteRef] {
        try db.pendingOutbox()
            ...
            .map { try HelixJSON.decoder.decode(RowDeleteRef.self, from: $0.payload) }
    }
    ...
    func clearingReachesTheQueue() throws {
        ...
        let ref = try HelixJSON.decoder.decode(RowRef.self, from: try db.pendingOutbox()[0].payload)
    ...
    func waterOverride() throws {
        ...
        let dayRef = try db.pendingOutbox()
            ...
            .map { try HelixJSON.decoder.decode(RowRef.self, from: $0.payload) }
