...
struct WidgetSnapshotBuilderTests {
    private let user = "u1"
    /// Thursday 2026-09-03 15:00 UTC — `cb_b` (Upper B) on the helix5 week.
    ...
    private func build(_ db: AppDatabase, _ scope: HelixScope) throws -> HelixSnapshot {
    ...
    func readOnly() throws {
        let folder = FileManager.default.temporaryDirectory.appending(path: "helix-ro-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: folder) }
        #expect(throws: AppDatabase.OpenError.missingDatabase(folder.appendingPathComponent("helix.sqlite").path)) {
    ...
    func migratesOldStore() throws {
        let base = FileManager.default.temporaryDirectory.appending(path: "helix-mv-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: base) }
        let old = base.appending(path: "old"), new = base.appending(path: "new")
        try FileManager.default.createDirectory(at: old, withIntermediateDirectories: true)
        for suffix in ["", "-wal", "-shm"] {
            try Data("x".utf8).write(to: old.appendingPathComponent("helix.sqlite" + suffix))
        }
        AppDatabase.moveStoreIfNeeded(from: old, to: new)
        for suffix in ["", "-wal", "-shm"] {
            #expect(FileManager.default.fileExists(atPath: new.appendingPathComponent("helix.sqlite" + suffix).path))
        ...
        try Data("y".utf8).write(to: old.appendingPathComponent("helix.sqlite"))
        AppDatabase.moveStoreIfNeeded(from: old, to: new)
        #expect(try String(contentsOf: new.appendingPathComponent("helix.sqlite"), encoding: .utf8) == "x")
    }
}
