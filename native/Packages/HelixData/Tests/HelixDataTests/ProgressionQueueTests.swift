...
struct ProgressionQueueTests {
    ...
    private var lift: (name: String, ceiling: Double, dayKey: String) {
        let day = Program.helix5.day(key: "legs_a")!
    ...
    func ready() throws {
        ...
        let queue = try db.progressionQueue(dayKey: l.dayKey, program: .helix5, phase: .cut, today: today)
    ...
    func scoped() throws {
        ...
        #expect(try db.progressionQueue(dayKey: "cb_a", program: .helix5, phase: .cut, today: today).isEmpty)
        #expect(try db.progressionQueue(dayKey: "nope", program: .helix5, phase: .cut, today: today).isEmpty)
    }
}
