...
///
///     HELIX_LIVE_URL=<host> HELIX_LIVE_ANON_KEY=<key> HELIX_LIVE_SESSION_FILE=<json with access_token + refresh_token>
///
...
@Suite("Live backfill gate", .enabled(if: ProcessInfo.processInfo.environment["HELIX_LIVE_SESSION_FILE"] != nil))
struct BackfillLiveTests {

    @Test("every table lands with exactly the server's row count")
    func countsMatch() async throws {
        let env = ProcessInfo.processInfo.environment
        let path = try #require(env["HELIX_LIVE_SESSION_FILE"])
        let json = try #require(try JSONSerialization.jsonObject(with: Data(contentsOf: URL(fileURLWithPath: path))) as? [String: Any])
        let client = HelixSupabase.makeClient(config: SupabaseConfig(
            url: try #require(URL(string: "https://" + (env["HELIX_LIVE_URL"] ?? ""))),
            anonKey: try #require(env["HELIX_LIVE_ANON_KEY"])
        ))
