...
export interface CustomSupplement {
  ...
  schedule: { days?: number[] } | null   // 0=Sun … 6=Sat; absent ⇒ every day
}
  ...
  return useMutation({
    mutationFn: async (s: NewCustomSupplement) => {
      ...
      const { error } = await supabase.from('custom_supplements').insert({
        ...
        form: s.form, time: s.time, schedule: { days: s.days },
      } as unknown as never)
 ...
 */
export function customSlotsForDate(customs: CustomSupplement[], weekday: number): SupplementSlot[] {
  ...
  return [...byTime.entries()]
    ...
    .map(([time, items]) => ({
      ...
      items: items.map((c) => ({ key: `custom:${c.id}`, name: c.name, dose: c.dose })),
    }))
}
