...
// The pure half of `useFatigue.ts` — vocabulary, levels, fold and summary —
// in a module with no 'use client' header, so the SERVER scorer
// (`computeForDate`) can read the day's latest reading through the same rule
// the tracker shows. Battery v8's stress term is the first consumer.
...
/** The old keys themselves, for a clear that must also remove a legacy row. */
export const LEGACY_SLOT_KEYS: readonly string[] = Object.keys(LEGACY_SLOTS)
