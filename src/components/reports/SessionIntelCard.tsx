...
function Chip({ label, value, accent = '#E2683A' }: { label: string; value: string; accent?: string }) {
  return (
    ...
    <div className="space-y-4">
      ...
      {insight && (
        <div className="rounded-2xl border px-3.5 py-3 flex gap-2.5 items-start"
          style={{ borderColor: '#E2683A33', background: '#E2683A0d', boxShadow: '0 0 18px #E2683A14' }}>
          <Sparkles className="w-4 h-4 shrink-0 mt-0.5" style={{ color: '#E2683A', filter: 'drop-shadow(0 0 4px #E2683A)' }} aria-hidden="true" />
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {session.avgBpm != null && <Chip label="Avg BPM" value={`${session.avgBpm}`} accent="#E2683A" />}
        <Chip label="Volume" value={`${Math.round(displayWeight(session.volumeKg ?? intel?.computedVolumeKg ?? 0) ?? 0).toLocaleString()} ${unit}`} accent="#9AA6B8" />
        <Chip label="Sets" value={`${session.setCount ?? intel?.computedSets ?? '—'}`} accent="#B84F28" />
      </div>
      ...
      {intel?.volumeDeltaPct != null && (
        <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] px-3 py-2 flex items-center gap-2 text-fluid-xs">
          ...
          <span className="helix-num font-bold" style={{ color: intel.volumeDeltaPct >= 0 ? '#4FB477' : '#D5514E' }}>
      ...
      {/* PR spotlight */}
      {!!intel?.prs.length && (
        <div className="rounded-2xl border px-3 py-2.5 space-y-1" style={{ borderColor: '#C9A22755', background: '#C9A22712', boxShadow: '0 0 18px #C9A22722' }}>
          {intel.prs.map((pr) => (
            <div key={pr.name} className="flex items-center gap-2 text-fluid-sm">
              <Star className="w-3.5 h-3.5 shrink-0" style={{ color: '#C9A227', filter: 'drop-shadow(0 0 4px #C9A227)' }} />
              <span className="text-text font-medium truncate">{pr.name}</span>
              <span className="helix-num ml-auto font-bold" style={{ color: '#C9A227' }}>{displayWeight(pr.kg)}{unit} × {pr.reps}</span>
            </div>
          ))}
        </div>
      )}
            ...
            <tbody>
              {intel.deltas.map((d) => (
                <tr key={d.name} className="border-b border-white/[0.06] last:border-0">
                  <td className="px-3 py-2.5 text-text/90 truncate max-w-[130px]">{d.name}{d.isPr && <Star className="inline w-3 h-3 ml-1 -mt-0.5" style={{ color: '#C9A227' }} />}</td>
              ...
              return (
                <div key={v.date} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-md" style={{
                    ...
                    background: isThis ? '#E2683A' : 'rgba(255,255,255,0.12)',
                    boxShadow: isThis ? '0 0 10px #E2683A66' : undefined,
                  }} />
