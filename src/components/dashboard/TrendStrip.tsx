...
function useTrendSeries() {
  return useQuery({
    ...
    queryFn: async (): Promise<Series[]> => {
      ...
      return [
        { label: 'Score', icon: Gauge, color: '#3E9E7A', points: scores.map((r) => r.score ?? 0).filter(Number.isFinite) },
        { label: 'Weight', icon: Scale, color: '#3E9E7A', unit: 'kg', points: body.map((r) => r.weight_kg ?? 0).filter((n) => n > 0) },
        { label: 'Sleep', icon: Moon, color: '#3D7AB8', unit: 'h', points: sleep.map((r) => Math.round(((r.duration_min ?? 0) / 60) * 10) / 10).filter((n) => n > 0) },
      ]
    },
    staleTime: 5 * 60_000,
  })
