...
export function SessionHero({ detail }: { detail: SessionDetail }) {
  ...
  return (
    <Surface variant="band" accent={accent} pad="snug" className="space-y-3">
      ...
      {/* ── THE START TIME IS NOT DOWN HERE ANY MORE ──
          It was a row of its own under the four `Sub` cells, behind a rule, with
          an uppercase label and the figure in full-strength `text-text` — so the
          brightest thing at the foot of a block of metrics was the one item in
          it that is not a metric, and the eye finished on it. It is the second
          half of "when", so it sits beside the date in `SessionTitle`, in the
          muted type that line already uses. */}
