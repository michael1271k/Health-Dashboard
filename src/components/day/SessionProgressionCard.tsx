...
export function SessionProgressionCard({ session, date }: { session: GymReportRow; date: string }) {
  ...
  return (
    <section className="helix-card holo-sheen space-y-3" style={{ borderColor: '#E2683A28' }}>
      <Link href={`/session/${session.id}`} onPointerUp={blurOnTap}
        ...
        <span className="w-7 h-7 rounded-full flex items-center justify-center shrink-0" style={{ background: '#E2683A1a', color: '#E2683A' }}>
        ...
        <span className="text-[10px] font-semibold uppercase tracking-wide shrink-0 flex items-center gap-0.5" style={{ color: '#9AA6B8' }}>
        ...
        {confirm ? (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              style={{ color: '#fff', background: '#D5514E', boxShadow: '0 0 16px #D5514E55' }}
            >
          ...
          <div className="flex items-center gap-2">
            <button type="button" disabled={edit.loading} onClick={() => edit.load(session.id)}
              className="btn-glass min-h-[38px] text-fluid-xs justify-center flex-1" style={{ color: '#9AA6B8' }}>
            ...
            <button type="button" onClick={() => setConfirm(true)}
              ...
              style={{ color: '#D5514E', background: '#D5514E1a', border: '1px solid #D5514E55' }}>
