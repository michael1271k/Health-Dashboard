...
const CHEST_PRESS: HistoryRow[] = [
  ...
  row({ name: 'Chest Press', date: '2026-08-23', dayKey: 'cb_a', n: 1, reps: 11 }),
  row({ name: 'Chest Press', date: '2026-08-23', dayKey: 'cb_a', n: 2, reps: 10 }),
  // 2026-08-20 · cb_b — THREE sets. The right one.
  row({ name: 'Chest Press', date: '2026-08-20', dayKey: 'cb_b', n: 1, reps: 11 }),
  row({ name: 'Chest Press', date: '2026-08-20', dayKey: 'cb_b', n: 2, reps: 10 }),
  row({ name: 'Chest Press', date: '2026-08-20', dayKey: 'cb_b', n: 3, reps: 10 }),
]
...
describe('previous-session memory is scoped to the routine', () => {
  ...
  it('fills set 3 — the cell that was blank on 2026-08-27', () => {
    const out = historyFromRows(CHEST_PRESS, { scopeKey: 'cb_b' })
    const third = out.get('Chest Press')!.sets[2]
  ...
  it('shows nothing rather than another routine on a movement\'s first outing', () => {
    ...
    expect(out.get('Chest Press')).toBeUndefined()
  })
  ...
  it('never reaches past a session it is being read for', () => {
    ...
    const withLater: HistoryRow[] = [
      row({ name: 'Chest Press', date: '2026-08-27', dayKey: 'cb_b', n: 1, reps: 12 }),
      ...CHEST_PRESS,
    ]
    const out = historyFromRows(withLater, { scopeKey: 'cb_b', before: '2026-08-27' })
    expect(out.get('Chest Press')!.date).toBe('2026-08-20')
  })
  ...
  it('orders sets by set_number, not by the arbitrary batch-insert order', () => {
    ...
    expect(out.get('Chest Press')!.sets.map((s) => s.reps)).toEqual([11, 10, 10])
  })

  it('carries the tag and the pair through, so seeding rebuilds the session', () => {
    const rows: HistoryRow[] = [
      row({ name: 'Single Arm Triceps Pushdown', date: '2026-08-20', dayKey: 'arms', n: 1, weight_kg: 7.5, reps: 13, side: 'L', pair_id: 'p1' }),
      row({ name: 'Single Arm Triceps Pushdown', date: '2026-08-20', dayKey: 'arms', n: 2, weight_kg: 7.5, reps: 13, side: 'R', pair_id: 'p1' }),
      row({ name: 'Single Arm Triceps Pushdown', date: '2026-08-20', dayKey: 'arms', n: 3, weight_kg: 6.25, reps: 10, set_type: 'failure' }),
    ]
    const sets = historyFromRows(rows, { scopeKey: 'arms' }).get('Single Arm Triceps Pushdown')!.sets
